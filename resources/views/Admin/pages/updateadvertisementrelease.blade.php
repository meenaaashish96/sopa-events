		...
		<div class="content-wrapper">
			...
			<div class="card">
				<div class="card-header">
					<h5 class="mb-0">Update Advertisement Release</h5>
				</div>
				

				<div class="card-body border-top">
					<form method="POST" action="{{url('sopa/admin/advertisement/release/edit/'.$advertisementrelease->id)}}" enctype="multipart/form-data">
						@csrf

						<input type="hidden" name="id" value="{{$advertisementrelease->id}}" class="form-control">
							...
							<select name="event_id" class="form-control form-control-select2" required>
								@if(count($events)>0)
									...
									@foreach($events as $key => $event)
										<option value="{{$event->id}}" {{$event->id == $advertisementrelease->event_id  ? 'selected' : ''}}>{{$event->title}}</option>
									@endforeach
								@else
						...
						<div class="mb-3">
							...
							<input type="text" name="organization_name" value="{{$advertisementrelease->organization_name}}" class="form-control" placeholder="Organization Name" required />
						</div>

						<div class="mb-3">
							<label class="form-label">Email<sup>*</sup></label>
							<input type="email" name="email" value="{{$advertisementrelease->email}}" class="form-control" placeholder="Email" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">GSTIN<sup>*</sup></label>
							<input type="text" style="text-transform: uppercase" name="GSTIN" value="{{$advertisementrelease->GSTIN}}" class="form-control" placeholder="GSTIN" required />
						</div>
						<div class="mb-3">
							<label class="form-label">Address<sup>*</sup></label>
							<input type="text" name="address" value="{{$advertisementrelease->id}}" class="form-control" placeholder="Address" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">State<sup>*</sup></label>
							<input type="text" name="state" value="{{$advertisementrelease->state}}" class="form-control" placeholder="State" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">City<sup>*</sup></label>
							<input type="text" name="city" value="{{$advertisementrelease->city}}" class="form-control" placeholder="City" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">Pin Code<sup>*</sup></label>
							<input type="number" min="1" max="999999" name="pin_code" value="{{$advertisementrelease->pin_code}}" class="form-control" placeholder="Pin Code" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">Tel Phone No.<sup>*</sup></label>
							<input type="number" min="1" name="tel_phone" value="{{$advertisementrelease->tel_phone}}" class="form-control" placeholder="Tel Phone" required />
						</div>
						
						<div class="mb-3">
							<label class="form-label">Mobile No.<sup>*</sup></label>
							<input type="number" min="1" name="mobile_phone" value="{{$advertisementrelease->mobile_phone}}" class="form-control" placeholder="Mobile Phone" required />
						</div>
							...
							<select name="hotel_room_reservation_id" class="form-control form-control-select2">
								@if(count($hotelroomreservation)>0)
									...
									@foreach($hotelroomreservation as $key => $room)
										<option value="{{$room->id}}"  {{$event->id == $advertisementrelease->hotel_room_reservation_id  ? 'selected' : ''}}>{{$room->organization_name}}</option>
									@endforeach
								@else
						...
						<div class="mb-3">
							...
							<select name="advertisement_id" class="form-control form-control-select2">
								@if(count($advertisements)>0)
									<option value="">Select Advertisement</option>
									@foreach($advertisements as $key => $room)
										<option value="{{$room->id}}"  {{$room->id == $advertisementrelease->advertisement_id  ? 'selected' : ''}}>{{$room->name}}</option>
									@endforeach
								@else
									<option value="">No Advertisement Found!</option>
								@endif
							</select>
						</div>
							...
							@foreach ($delegets as $key => $deleget)
								...
								<div class="sections{{$key}}">
									...
									<div class="row pb-3">
										...
										<div class="form-group col" style="align-items: flex-end;display: flex;">
											<button onclick="remove({{$key}}, {{$deleget->id}}, {{$advertisementrelease->id}})"  type="button" name="add" id="add"  class="btn btn-primary"> Remove </button>
						...
						<div class="mb-3">
							...
							<select name="payment_mode" class="form-control form-control-select2">
								...
								<option value="offline"  {{"offline" == $advertisementrelease->payment_mode  ? 'selected' : ''}}>Offline</option>
								<option value="online"  {{"online" == $advertisementrelease->payment_mode  ? 'selected' : ''}}>Online</option>
							</select>
						</div>
						...
						<div class="mb-3">
							...
							<input type="text" name="UTR_number" value="{{$advertisementrelease->UTR_number}}" class="form-control" placeholder="UTR Number" required />
						</div>

						<div class="mb-3">
							<label class="form-label">Cheque/Recipt Number<sup>*</sup></label>
							<input type="text" name="Cheque_receipt_number" value="{{$advertisementrelease->Cheque_receipt_number}}" class="form-control" placeholder="Cheque/Recipt Number" required />
						</div>
						...
						<div class="mb-3">
							...
							<input type="datetime-local" name="payment_date" value="{{$advertisementrelease->payment_date}}" class="form-control" placeholder="payment date" required />
						</div>
